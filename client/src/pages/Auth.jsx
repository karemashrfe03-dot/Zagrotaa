...
export default function Auth() {
    ...
    const [signupServiceType, setSignupServiceType] = useState("VENUE");

    // Makeup Artist specific fields
    const [makeupPackagePrice, setMakeupPackagePrice] = useState("");
    const [makeupBrandsUsed, setMakeupBrandsUsed] = useState("");
    const [makeupHijabFriendly, setMakeupHijabFriendly] = useState(false);
    const [makeupYearsOfExperience, setMakeupYearsOfExperience] = useState("");
    const [makeupInstagram, setMakeupInstagram] = useState("");
      ...
      try {
        const res = await fetch(`${API_URL}/users/register`, {
          ...
          body: JSON.stringify({
             ...
             serviceType: signupRole === "VENDOR" ? signupServiceType : undefined,
             makeupDetails: signupRole === "VENDOR" && signupServiceType === "MAKEUP_ARTIST" ? {
               packagePrice: makeupPackagePrice ? parseInt(makeupPackagePrice, 10) : 0,
               brandsUsed: makeupBrandsUsed,
               hijabFriendly: makeupHijabFriendly,
               yearsOfExperience: makeupYearsOfExperience ? parseInt(makeupYearsOfExperience, 10) : 0,
               instagramPortfolio: makeupInstagram
             } : undefined
          })
        });
          ...
          <div className="p-8">
            ...
            {tab === 'signup' && (
              <form onSubmit={handleSignup} className="space-y-5 animate-in fade-in zoom-in-95 duration-200">
                ...
                {signupRole === 'VENDOR' && (
                  <div>
                    <label className="block text-sm font-semibold mb-2">نوع الخدمة</label>
                    <CustomSelect 
                      value={signupServiceType}
                      onChange={(e) => setSignupServiceType(e.target.value)}
                      options={[
                        { value: "VENUE", label: "قاعة أفراح" },
                        { value: "ATELIER", label: "أتيليه فساتين" },
                        { value: "MAKEUP_ARTIST", label: "ميك أب آرتيست" }
                      ]}
                      className="p-3 border border-gray-200 rounded-xl bg-gray-50 focus:ring-2 focus:ring-[#8c71af] font-bold text-gray-700 hover:border-[#8c71af] transition"
                    />
                  </div>
                ...
                {signupRole === 'VENDOR' && signupServiceType === 'MAKEUP_ARTIST' && (
                  <div className="space-y-4 bg-pink-50/50 p-4 rounded-xl border border-pink-100">
                    <h3 className="font-bold text-[#8c71af] mb-4">تفاصيل الميك أب آرتيست</h3>
                    
                    <div>
                      <label className="block text-sm font-semibold mb-2">سعر الباكيج (يبدأ من)</label>
                      <input 
                        type="number" 
                        required
                        value={makeupPackagePrice}
                        onChange={(e) => setMakeupPackagePrice(e.target.value)}
                        placeholder="مثال: 5000" 
                        className="w-full p-3 border border-gray-200 rounded-xl bg-white focus:ring-2 focus:ring-[#8c71af] outline-none transition"
                      />
                    </div>

                    <div>
                      <label className="block text-sm font-semibold mb-2">أنواع البراندات المستخدمة</label>
                      <textarea 
                        required
                        value={makeupBrandsUsed}
                        onChange={(e) => setMakeupBrandsUsed(e.target.value)}
                        placeholder="مثال: MAC, Huda Beauty, NARS" 
                        className="w-full p-3 border border-gray-200 rounded-xl bg-white focus:ring-2 focus:ring-[#8c71af] outline-none transition resize-none h-24"
                      ></textarea>
                    </div>
                    ...
                    <div>
                      ...
                      <input 
                        type="number" 
                        required
                        value={makeupYearsOfExperience}
                        onChange={(e) => setMakeupYearsOfExperience(e.target.value)}
                        placeholder="مثال: 5" 
                        className="w-full p-3 border border-gray-200 rounded-xl bg-white focus:ring-2 focus:ring-[#8c71af] outline-none transition"
                      />
                    </div>
                    ...
                    <div>
                      <label className="block text-sm font-semibold mb-2">رابط انستجرام لأعمال سابقة</label>
                      <input 
                        type="url" 
                        value={makeupInstagram}
                        onChange={(e) => setMakeupInstagram(e.target.value)}
                        placeholder="https://instagram.com/..." 
                        className="w-full p-3 border border-gray-200 rounded-xl bg-white focus:ring-2 focus:ring-[#8c71af] outline-none transition text-left"
                        dir="ltr"
                      />
                    </div>

                    <label className="flex items-center gap-3 cursor-pointer p-3 border border-gray-200 rounded-xl bg-white hover:border-[#8c71af] transition">
                      <input 
                        type="checkbox" 
                        checked={makeupHijabFriendly}
                        onChange={(e) => setMakeupHijabFriendly(e.target.checked)}
                        className="accent-[#8c71af] w-5 h-5"
                      />
                      <span className="font-semibold text-gray-700">متاح للمحجبات (حجاب فري)</span>
                    </label>
                  </div>
                )}
                <div>
